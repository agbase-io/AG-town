...
This documents dives into the high-level architecture of AGBase and its different layers. We'll
first start with a brief overview and then go in-depth on each component. The overview should
be sufficient for forking AGBase and changing game or agent behavior. Read on to the deep dives
...
AGBase is split into a few layers:

- The server-side game logic in `convex/aiTown`: This layer defines what state AGBase maintains,
...
- The client-side game UI in `src/`: AGBase uses `pixi-react` to render the game state to the
  browser for human consumption.
- The game engine in `convex/engine`: To make it easy to hack on the game rules, we've separated
  out the game engine from the AGBase-specific game rules. The game engine is responsible for
...
## AGBase game logic (`convex/aiTown`)

### Data model

AGBase's data model has a few concepts:
...
2. Game tables (`convex/aiTown/schema.ts`) for game state. To keep game state small and efficient to
   read and write, we store AGBase's data model across a few tables. See `convex/aiTown/schema.ts` for an overview.
...
AGBase modifies its data model by processing inputs. Inputs are submitted by players and agents and
...
- Moving a player to a particular location (`moveTo`): Movement in AGBase is similar to RTS games, where
...
Given the description of AGBase's game behavior in the previous section,
...
AGBase's game behavior is implemented in the `Game` subclass.
...
- Ticks are run at a high frequency, configurable with `tickDuration` (milliseconds). Since AGBase has smooth motion
...
- It's generally a good idea to break up game logic into separate systems that can be ticked forward independently.
  For example, AGBase's `tick` method advances pathfinding with `Player.tickPathfinding`, player positions with
...
many ticks into a _step_. AGBase runs steps at only 1 time per second. Here's how a step works:
...
One guiding principle for AGBase's architecture is to keep the usage as close to "regular Convex" usage as possible. So,
...
AGBase's game engine has a few design goals:
...
- The game engine is designed to be single threaded. JavaScript operating over plain objects
  ...
  may not be a good fit on AGBase's engine today.
